...
const ApiError = require("../utils/ApiError");
const { createAgreement } = require("./agreement.service");
const { generateSecureToken } = require("../utils/token");
const {
  generateLeaseNumber,
  getActiveLeaseForUnit,
...
const onboardTenant = async ({
  ...
  // Unit must be fully vacant — not just unoccupied
  // A PENDING lease means the unit is being processed, block a second onboarding
  if (unit.status === "Occupied") {
    throw new ApiError(400, "This unit is already occupied");
  }
  if (unit.status === "Under Maintenance") {
  ...
  // ── 3. Block if a pending or active lease already exists on the unit ──────
  const existingLease = await getActiveLeaseForUnit(unitId);
  if (existingLease) {
    throw new ApiError(
      400,
      "This unit already has a lease in progress. Complete or cancel it before onboarding a new tenant.",
    );
  }

  // ── 4. Check overlapping dates ────────────────────────────────────────────
  const overlap = await hasOverlappingLease(
    unitId,
    new Date(startDate),
    new Date(endDate),
  ...
  // ── 8. Create Lease — status PENDING, unit stays VACANT ──────────────────
  const leaseNumber = await generateLeaseNumber();
  const lease = await Lease.create({
    leaseNumber,
    tenant: tenant._id,
    ...
    // Unit is NOT touched here — stays Vacant until both parties sign
  });

  // ── 9. Create RentalAgreement + generate PDF ──────────────────────────
  const agreement = await createAgreement(lease._id);

  // ── 10. Generate activation token + invitation ────────────────────────────
  const { plain: activationToken, hash: tokenHash } =
    await generateSecureToken();
  ...
  await logActivity({
    ...
    meta: {
      ...
      unitNumber: unit.unitNumber,
      propertyName: property.name,
      note: "Lease is PENDING — unit remains VACANT until agreement is fully signed",
    },
  });

  return { tenant, lease, agreement, invitation };
};
